...
namespace Serial
{
    class MainClass
    {
        ...
        public static void TestNeuralNetwork()
        {
            ...
            csv.GetFiles();
            NeuralNetwork1.NeuralNetwork nn = new NeuralNetwork1.NeuralNetwork(0.2, new int[] { 30000, 1000, 1000, 1000, 2 });


            for (int i = 0; i < csv.AccDataList.Count; i++)
            {
                for (int index = 0; index < csv.AccDataList[i].AccelerationData.Count / 100 - 49; index++)
                {
                    List<double> input = new List<double>();
                    List<double> output = new List<double>();


                    foreach (XYZ xyz in csv.AccDataList[i].GetRunningAverage(200, true).GetRange(index * 100, 5000))
                    {
                        input.Add(xyz.X);
                        input.Add(xyz.Y);
                        input.Add(xyz.Z);
                    ...
                    foreach (XYZ xyz in csv.GyroDataList[i].GetNormalized().GetRange(index * 100, 5000))
                    {
                        input.Add(xyz.X);
                        input.Add(xyz.Y);
                        input.Add(xyz.Z);
                    ...
                    {
                        List<XYZ> pos = csv.PozyxDataList[i].GetNormalized().GetRange(index * 100, 5000);
                        output.Add(pos.Last().X - pos.First().X);
                        output.Add(pos.Last().Y - pos.First().Y);
                    }
                    Console.WriteLine($"Træning nr.: {i}");
